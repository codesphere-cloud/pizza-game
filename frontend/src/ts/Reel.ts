...
export class Reel {
    private readonly reelContainer: Element;
    private readonly idx: number;
    private readonly symbolContainer: HTMLElement;
    private readonly animation: Animation;
    private readonly factor: number;

    constructor(reelContainer: Element, idx: number, initialSymbols: string[]) {
        this.reelContainer = reelContainer;
        this.idx = idx;
        this.factor = 1 + Math.pow(this.idx / 2, 2);
        ...
        initialSymbols.forEach((symbol) =>
            this.symbolContainer.appendChild(new Symbol(symbol).getImg())
        );
    }
    ...
    public renderSymbols(nextSymbols: string[]): void {
        ...
        for (let i = 3; i < 3 + Math.floor(this.factor) * 10; i++) {
            ...
            fragment.appendChild(icon.getImg());
        }
